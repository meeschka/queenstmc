...
class SteadyStateClass(SysVarDigital):
    def __init__(self, name, states, controllerClass):
        ...
        self.temps = [1,4,7,8,9,10]                         #list for all temperatures, updated one row at a time
    ...
    def GetFunc(self):
        ...
        if len(self.Data) > 0:            #if there is data that has not been put into temps list
            ...
            for idx in reversed(range(1,self.index)):          #from index back to beginning
                for col in self.temps:                            #check all columns
                    diff = abs(float(self.Data[self.index][col])-float(self.Data[idx][col])) #difference between current row index and most recently added index
