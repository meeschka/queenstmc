...
//if want oversampling, put that chunk here
...
void cmdRead(int argC, char **argV) {
  ...
  for (int i = 1; i <= argC; i++) {
    ...
    } else if (strcasecmp(pinType, "B") == 0 ) {
      //read from first adc
      value = ads1.readADC_SingleEnded(pin);
    } else if (strcasecmp(pinType, "C") == 0 ) {
      //read from second adc
      value = ads2.readADC_SingleEnded(pin);
    }    else {     
      return;
    }
  ...
  for (i = 0; i < PID_RELAY_MAX_VARS; i++) {
    if (pidRelayDescs[i].isOn) {
      ...
      if (pidRelayDescs[i].outputVar > curMs - pidRelayDescs[i].windowStartTime) {
        ...
        //switched low and high because relay is low-activated, so high = off
      }
    }
  }
#endif
