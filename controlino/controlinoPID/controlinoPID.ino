...
void loop() {
	...
	if (Serial.available()) {
		...
		switch (c) {
		case '\n':  //whenever there is a new line, break
			break;    
		case '\r':        //whenever 'carriage return' is used, go through switch
			// end the string and init pMsg

			*(pMsg++) = NULL;        //no longer need pointer to pMsg++
			pMsg = msg;
			// parse the command line statement and break it up into space-delimited
			// strings (strtok). the array of strings will be saved in the argV array.
			i = 0;
			argV[i] = strtok(msg, " ");
			...
			argC = i;
			pin = strtol(argV[1], NULL, 10);           //seperates into string and numbers. so p=int argV[1], null string, base 10 
                        //Serial.println(argV[0], argV[1], argV[2]);
